...
Class Users extends DBConnection {
	...
	public function save_vendor(){
		...
		if(empty($_POST['id'])){
			...
			$code = sprintf("%'.05d",1);
			while(true){
				$check = $this->conn->query("SELECT * FROM `seller` where code = '{$prefix}{$code}'")->num_rows;
				if($check > 0){
		...
		if(isset($oldpassword) && !empty($id)){
			$current_pass = $this->conn->query("SELECT * FROM `seller` where id = '{$id}'")->fetch_array()['password'];
		...
		$check  = $this->conn->query("SELECT * FROM `seller` where username = '{$username}' and delete_flag = 0 ".(!empty($id) ? " and id !='{$id}'" : ''))->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = " Username already exists";
		}else{
			if(empty($id)){
				$sql = "INSERT INTO `seller` set {$data}";
			}else{
				$sql = "UPDATE `seller` set {$data} where id = '{$id}'";
			}
				...
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					...
					if(!in_array($type,$allowed)){
						...
						if($gdImg){
								...
								imagedestroy($t_image);
								if($uploaded_img){
									$qry = $this->conn->query("UPDATE `seller` set avatar = concat('{$fname}','?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '$vid' ");
	...
	public function delete_vendor(){
		extract($_POST);
		$qry = $this->conn->query("UPDATE seller set delete_flag = 1 where id = $id");
		if($qry){
