...
class Login extends DBConnection {
	...
	public function login_vendor(){
		extract($_POST);
		$password = md5($password);
		$stmt = $this->conn->prepare("SELECT * from seller where username = ? and `password` =? and delete_flag = ?  ");
		$delete_flag = 0;
	...
	public function login_client(){
		extract($_POST);
		$password = md5($password);
		$stmt = $this->conn->prepare("SELECT * from customer where email = ? and `password` =? and delete_flag = ?  ");
		$delete_flag = 0;
